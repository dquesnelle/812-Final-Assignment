...
Rscript ./2-MSA/msa.R $infile $outfile
# Rscript ./2-MSA/msa.R "nifh_sequences1.fasta" "./2-MSA/out.fasta" # arguments are infile and outfile

# for testing, use next line:
# Rscript ./2-MSA/msa.R "./test-files/test_sequences.fasta" "./test-files/testout.fasta"
mview.bat -in fasta -width 100 -html head -bold -css on -coloring identity -moltype dna $outfile > ./test-files/testfig.html
